/*****************************************************************************
    ...
    New allocation algorithm based on VBV fullness only
...
void dumpProgs()
{
    ...
    for (i=0; i<progCnt; i++) {
        ...
        printf("\nPID %d: schOut %d, VBV %d",
            pProg->pid, pProg->schOutPn, pProg->schVbvLevel);
    }
}
...
ProgInfo* selectProg()
{
    int i;
    int idx = -1;
    int minVbvLevel = 1<<30;	// some large number
    ProgInfo* pProg;
...
//                pProg->pid, pProg->schVbvLevel);
        ...
        if (pProg->schVbvLevel < minVbvLevel) {
            minVbvLevel = pProg->schVbvLevel;
            idx = i;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    else  readParam(&par, "statmux5.par");
