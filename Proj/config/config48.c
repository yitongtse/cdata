...
void ucast_session_config (int slot)
{
    int port, chan, qb;
    int i, n;
    ...
    for (port = 1; port <= 12; port++) {
        ...
        for (chan = 1; chan <= 4; chan++) {
            ...
            for (i = 1; i <= 10; i++, n++) {
                printf("cable video ip 192.168.%d.%d0 udp %d program %d\n",
                       slot, qb, n, i);
            }
        }
    }
...
void mcast_session_config (int slot)
{
    int port, chan, qb;
    int i, n;
...
//    ucast_session_config(7);

//    mcast_label_config();
//    mcast_session_config(6;
