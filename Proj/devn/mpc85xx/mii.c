...
uint16_t mpc_mii_read(void *handle, uint8_t phy_add, uint8_t reg_add)
{
    ...
    *(base + MPC_MIIMADD) = (MIIMADD_PHYADD(phy_add) |
                             MIIMADD_REGADD(reg_add));
...
#if 0
    printf("\tmpc_mii_read() : "
           "handle 0x%x, phy_add 0x%x, reg_add 0x%x, data 0x%08x\n",
           handle, phy_add, reg_add, val);  // WAYNE
#endif
...
void mpc_mii_write(void *handle, uint8_t phy_add, uint8_t reg_add,
                   uint16_t data)
{
    mpc85xx_t *ext = (mpc85xx_t *) handle;
    volatile uint32_t *base = ext->phy_reg;
...
#if 1
    printf("\tmpc_mii_write(): "
           "handle 0x%x, phy_add 0x%x, reg_add 0x%x, data 0x%08x  W\n",
           handle, phy_add, reg_add, data);  // WAYNE
#endif
}
...
void mpc_mii_callback(void *handle, uchar_t phy, uchar_t newstate)
{
    ...
    char *func_name = __FUNCTION__"()";  // WAYNE
    ...
    case MDI_LINK_UP:
        printf("%s: call MDI_GetActiveMedia\n", func_name); // WAYNE
        if ((i = MDI_GetActiveMedia(ext->mdi, ext->cfg.phy_addr, &mode)) !=
            MDI_LINK_UP) {
            if (i == MDI_LINK_DOWN) {
                if (ext->cfg.verbose) {
                    nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, "devn-ext: "
                              "callback GetActiveMedia returned LINK DOWN");
                }
                cfg->flags |= NIC_FLAG_LINK_DOWN;
                printf("%s: call MDI_AutoNegotiate\n", func_name);  // WAYNE
                MDI_AutoNegotiate(ext->mdi, ext->phy_addr, NoWait);
            } else {
                if (ext->cfg.verbose) {
                    nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, "devn-ext: "
                              "callback GetActiveMedia returned %x", i);
                    mode = 0;
                }
            }
        }
        printf("\tMDI_LINK_UP: ");  // WAYNE
        ...
        if (cfg->media_rate == 1000000) {
            // If its 1Gb
            *(base + MPC_MACCFG2) =
                ((*(base + MPC_MACCFG2) & ~MACCFG2_IF) | MACCFG2_IF_MODE_BYT);
        } else {
            // 10/100Mb
            *(base + MPC_MACCFG2) =
                ((*(base + MPC_MACCFG2) & ~MACCFG2_IF) | MACCFG2_IF_MODE_NIB);
        ...
        if (cfg->verbose) {
            nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, "Link up %d (%s)",
                      cfg->lan, s);
        }
    ...
    case MDI_LINK_DOWN:
        ...
        cfg->flags |= NIC_FLAG_LINK_DOWN;
        printf("%s: call MDI_AutoNegotiate", func_name);  // WAYNE
        MDI_AutoNegotiate(ext->mdi, ext->phy_addr, NoWait);
        printf("\tMDI_LINK_DOWN\n");  // WAYNE
        break;
...
int mpc_init_phy(mpc85xx_t *ext, int speed)
{
    ...
    uint32_t maccfg2;
    int timeout = MPC_TIMEOUT;
    int mii_read_cnt, mii_write_cnt; // WAYNE
    char *func_name = __FUNCTION__"()";  // WAYNE

    mii_read_cnt = mii_write_cnt = 0;  // WAYNE

    phy_idx = ext->phy_addr;
    if (cfg->verbose) {
        nic_slogf(_SLOGC_NETWORK, _SLOG_INFO,
                  "mpc_init_phy: speed: %d, duplex: %d, PHY: %d", speed,
                  cfg->duplex, ext->phy_addr);
    }
    printf("%s: speed: %d kbps, duplex: %d, PHY: %d\n",
           func_name, speed, cfg->duplex, ext->phy_addr);  // WAYNE

    memset(&mdi_event, 0, sizeof (struct sigevent));
    mdi_event.sigev_coid = ext->coid;
    printf("%s: call MDI_Register_Extended\n", func_name);  // WAYNE
    ...
    *(phy_base + MPC_MIIMCFG) = 3;
    ...
    while ((*(phy_base + MPC_MIIMIND) & MIIMIND_BUSY)) {
        ...
        if (!--timeout) {
            nic_slogf(_SLOGC_NETWORK, _SLOG_DEBUG1,
                      "MII timeout during init.  Continuing anyway...");
            break;
        }
    }

    printf("%s: call MDI_FindPhy\n", func_name);  // WAYNE
    if ((i = MDI_FindPhy(ext->mdi, ext->phy_addr)) == MDI_SUCCESS) {
        printf("%s: call MDI_InitPhy\n", func_name);  // WAYNE
        if ((i = MDI_InitPhy(ext->mdi, ext->phy_addr)) == MDI_SUCCESS) {
            printf("%s(): call MDI_ResetPhy\n", func_name);  // WAYNE
            if (cfg->verbose) {
                nic_slogf(_SLOGC_NETWORK, _SLOG_INFO,
                          "devn-mpc85xx: PHY found at address %d.",
                          ext->phy_addr);
            }
            MDI_ResetPhy(ext->mdi, ext->phy_addr, WaitBusy);

            if (PPC_GET_FAM_MEMBER(get_pvr() == PPC_E500) &&
                PPC_GET_REVISION(get_pvr()) < 2) {
                /* reset errata */
                if (cfg->verbose) {
                    nic_slogf(_SLOGC_NETWORK, _SLOG_INFO,
                              "devn-mpc85xx: Reset PHY errata being run.");
                }
                ...
                mii_write_cnt += 5;  // WAYNE
            }
        } else {
            printf("%s: can NOT init Phy\n", func_name);  // WAYNE
            if (cfg->verbose) {
                nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, "devn-mpc85xx: "
                          "PHY %d not initialized (InitPhy returned %d).",
                          ext->phy_addr, i);
            }
        }
    } else {
        printf("%s: can NOT find Phy\n", func_name);  // WAYNE
        if (cfg->verbose) {
            nic_slogf(_SLOGC_NETWORK, _SLOG_INFO,
                      "devn-mpc85xx: PHY %d not found (FindPhy returned %d).",
                      ext->phy_addr, i);
        }
    }
    ...
    if (negotiate) {
        ...
        cfg->flags |= NIC_FLAG_LINK_DOWN;
        if ((i = MDI_AutoNegotiate(ext->mdi, ext->phy_addr, NoWait)) !=
            MDI_SUCCESS) {
            if (cfg->verbose) {
                nic_slogf(_SLOGC_NETWORK, _SLOG_INFO,
                          "MDI_AutoNegotiate returned %x", i);
            }
        }

        printf("%s: call MDI_EnableMonitor\n", func_name);  // WAYNE
        if ((i = MDI_EnableMonitor(ext->mdi, 1)) != MDI_SUCCESS) {
            if (cfg->verbose) {
                nic_slogf(_SLOGC_NETWORK, _SLOG_ERROR,
                          "MDI_EnableMonitor returned %x", i);
            }
        }
    } else {
        printf("%s: NO auto negotiation\n", func_name);  // WAYNE
        an_capable = mpc_mii_read(ext, cfg->phy_addr, MDI_BMSR) & 8;
        mii_read_cnt += 1;  // WAYNE
        ...
        if (ext->force_advertise != -1 || !an_capable) {
            ...
            reg &=
                ~(BMCR_RESTART_AN | BMCR_SPEED_100 | BMCR_SPEED_1000 |
                  BMCR_FULL_DUPLEX);
            ...
            if (cfg->media_rate == 100 * 1000) {
                ...
                if (cfg->media_rate == 1000 * 1000) {
                    ...
                    an_capable = 0;
                    mcsr = ((cfg->duplex > 0) ?
                            MSCR_ADV_1000bTFD : MSCR_ADV_1000bT);
                } else {
    ...
    // WAYNE
    printf("%s: access mpc_mii_read() %d times\n", func_name, mii_read_cnt);
    printf("%s: access mpc_mii_write() %d times\n", func_name, mii_write_cnt);
    return (phy_idx);
}
