...
typedef struct {
    ...
    int iid[3];
    int version;
    uintptr_t iobase;         // phy addr of Controller ID Register (TSEC_ID)
    uintptr_t phy_base;       // phy addr of the 1st (primary) TSEC_ID
    uint32_t *reg;            // logic addr of TSEC_ID
    uint32_t *phy_reg;        // logic addr of the 1st (primary) TSEC_ID
    ...
    uint32_t probe_phy;   /* Whether or not the PHY should be
                           * constantly monitored for speed/duplex
                           * change */
    uint32_t use_syspage; /* Whether or not the system page should 
                           * be read for TSEC hardware information. */
} mpc85xx_t;
