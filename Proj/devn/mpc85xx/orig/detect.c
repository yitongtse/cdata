...
error:
		nic_slogf(_SLOGC_NETWORK, _SLOG_WARNING, "devn-mpc85xx: unknown option %s", c);
		err = EINVAL;
		rc = -1;
	}
...
static int	mpc_create_instance (void *dll_hdl, io_net_self_t *ion,
    char *options, int idx, uint32_t iobase, struct tsec_params *tparms_p)

{
	...
	if ((ext->reg = mmap_device_memory (NULL, 0x1000, PROT_READ | PROT_WRITE | PROT_NOCACHE,
		...
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx: mmap memory failed");
		return (-1);
	}
	if ((ext->phy_reg = mmap_device_memory (NULL, 0x1000, PROT_READ | PROT_WRITE | PROT_NOCACHE,
		MAP_SHARED, ext->phy_base)) == MAP_FAILED) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx: mmap PHY memory failed");
		return (-1);
	}
	...
	if (tparms_p != NULL) {
		if (cfg->verbose) {
			nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, 
				"devn-mpc85xx: IF %d: Base register 0x%08X", idx,tparms_p->paddr);
			nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, 
			...
			if (tparms_p->mac_present) {
				nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, 
						"devn-mpc85xx: IF %d: MAC %02X:%02X:%02X:%02X:%02X:%02X", idx,
				tparms_p->mac[0],tparms_p->mac[1],tparms_p->mac[2],tparms_p->mac[3],
				tparms_p->mac[4],tparms_p->mac[5]);
			}
			nic_slogf(_SLOGC_NETWORK, _SLOG_INFO, 
...

{
	...
	if ((iobase = find_immr_info ()) == 0) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx: Unable to find base address");
		return (ENODEV);
	}
	...
	if (ext->use_syspage) {
		...
		if (config_offset == HWI_NULL_OFF) {
			nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, 
					"devn-mpc85xx: No syspage hardware configuration information found");
			return(ENODEV);
		}
		...
		while (config_offset != HWI_NULL_OFF) {
			if (tsec.paddr && tsec.irq_present) {
				...
				if ((cfg.device_index == idx) || (cfg.device_index == 0xffffffff)) {
					if (mpc_create_instance (dll_hdl, ion, options, idx, iobase, &tsec) != EOK) {
						return (ENODEV);
					}
				...
				nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, 
					"devn-mpc85xx: Missing %s in syspage configuration for device index %d", missing, idx);
		...
		while (idx < ntsec) {
			...
			if (mpc_create_instance (dll_hdl, ion, options, idx, iobase, NULL) != EOK) {
				return (ENODEV);
			}
