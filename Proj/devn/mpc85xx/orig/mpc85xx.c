...
static int	mpc_process_interrupt (mpc85xx_t *ext, int id) {
	...
	for (;;) {
		...
		*(base + MPC_IEVENT) = ievent;
		if ((ievent & (IEVENT_RXC | IEVENT_RXBO | IEVENT_RXFO | IEVENT_BSY)) != 0) {
...

{
	...
	while (1) {
		...
		switch (pulse.code) {
		  ...
		  case MDI_TIMER:
			if (ext->probe_phy || 
					(ext->cfg.flags & NIC_FLAG_LINK_DOWN) || 
					!ext->rcvpkts) {
...
#if 0
	if ((ext->tx_addr_table = malloc (ext->num_tx_descriptors * sizeof(uint32_t))) == NULL) {
...
static int	mpc_init_list (mpc85xx_t *ext)
{
	...
	if ((npkt = mpc_alloc_npkt(ext, MPC_MTU_SIZE, ext->num_rx_descriptors)) == NULL) {
		errno = ENOBUFS;
		return -1;
	}
	
	for (i = 0, bd = (mpc_bd_t *) ext->rx_bd; i < ext->num_rx_descriptors; i++, bd++) {
...

{
	...
	if (nic_get_syspage_mac (cfg->permanent_address) == -1) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx: MAC address not found in system page");
//		read_phys_addr (ext);
	}
	...
	*(base + MPC_DMACTRL) |= (DMACTRL_WWR | DMACTRL_TBDSEN | DMACTRL_TDSEN /*| DMACTRL_WOP*/);
	...
	*(base + MPC_IMASK) = (IMASK_TXE | IMASK_TXB | IMASK_TXF | IMASK_RXBO | IMASK_RXFO | IMASK_BABR | IMASK_BSY | IMASK_EBERR);
	
	*(base + MPC_TSTAT) = TSTAT_THLT;	
	*(base + MPC_RSTAT) = RSTAT_QHLT;

	*(base + MPC_DMACTRL) &= ~(DMACTRL_GRS | DMACTRL_GTS);
	*(base + MPC_MACCFG1) = (MACCFG1_RXEN | MACCFG1_TXEN | ext->flowctl_flag | ext->loopback);
...

{
	...
	if ((ext->coid = ConnectAttach (0, 0, ext->chid, _NTO_SIDE_CHANNEL, 0)) < 0) {
		nic_slogf( _SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx:  Unable to ConnectAtttach" );
		goto error;
	}
	err++;
	...
	if (mpc_config (ext) != EOK) {
		nic_slogf ( _SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx:  mpc_config error" );
		goto error;
	}
	...
	if (pthread_mutex_init (&ext->tx_mutex, &mattr) != EOK) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx:  Unable to initialize TX mutex");
		goto error;
	}
	err++;

	if (pthread_mutex_init (&ext->rx_free_pkt_q_mutex, &mattr) != EOK) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "devn-mpc85xx:  Unable to initialize RX Free Queue mutex");
		goto error;
	}
	err++;
	...
	if ((ret = pthread_create (&ext->tid, &pattr,(void *) mpc_driver_thread, ext)) != EOK) {
		errno = ret;
	...
	if ((ext->iid [0] = InterruptAttachEvent (cfg->irq [0], &event, _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
	...
	if ((ext->iid [1] = InterruptAttachEvent (cfg->irq [1], &event, _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "InterruptAttachEvent at \"%s\":%d: %s\n",
			__FILE__, __LINE__, strerror(errno));
		goto error;
	}
	...
	if ((ext->iid [2] = InterruptAttachEvent (cfg->irq [2], &event, _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
		nic_slogf (_SLOGC_NETWORK, _SLOG_ERROR, "InterruptAttachEvent at \"%s\":%d: %s\n",
			__FILE__, __LINE__, strerror(errno));
		goto error;
	}
...
unsigned 
crc32core(unsigned char *buf, int len)
{
...
unsigned 
reflect(unsigned val, int nbits)
{
    unsigned ret = 0;
    int k;
    for (k=1; k < (nbits+1); k++){
...
void	
setup_mcast_range (mpc85xx_t *ext, unsigned char *da, uint64_t len, int on)
{
	unsigned 	crc;
	unsigned 	lsb;
	unsigned	bitIndex;
...
void	
mcast_prom (mpc85xx_t *ext, int enable)
{
	int			 I;
	uint32_t	*gaddr_reg = ext->reg + MPC_GADDR0;
...
int		
do_multicast (mpc85xx_t *ext, struct _io_net_msg_mcast *mcast)
{
	...
	if (mcast->type == _IO_NET_REMOVE_MCAST) {
		...
		if (ext->cfg.verbose) {
		    nic_slogf(_SLOGC_NETWORK, _SLOG_ERROR,
		        "devn-mpc85xx: message 0x%x not supported",
		        mcast->type);
		}
		return EINVAL;
 	}
