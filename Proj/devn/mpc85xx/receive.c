...
void mpc_receive(mpc85xx_t * ext)
{
    ...
    while (1) {
        ...
        // WAYNE vvv
        // printf("%s() >> Rx BD 0x%08x, Pckt 0x%08x (%d byte, status 0x%x)\n",
        //        __FUNCTION__, vtophys((void *) rx_bd),
        //        rx_bd->buffer, rx_bd->length, status);

        if (status & RXBD_ERR) {
            nic_slogf(_SLOGC_NETWORK, _SLOG_ERROR,
                      "devn-mpc85xx: status = %08x\n", status);

            /* Return the packet to the receive queue.  No "receive
             * complete" required since the packet wasn't used. */
            mpc_add_pkt(ext, npkt, cidx);
            printf("%s() >> RXBD_ERR, status = %08x\n", __FUNCTION__, status);
            continue;
        }
        ...
        if ((new = ext->rx_free_pkt_q)) {
            ...
            if (!(new = mpc_alloc_npkt(ext, MPC_MTU_SIZE, 1))) {
                errno = ENOBUFS;
                nic_slogf(_SLOGC_NETWORK, _SLOG_ERROR,
                          "devn-mpc85xx: Packet Allocation failure.");
                printf("%s() >> Packet Allocation failure\n", __FUNCTION__);
                break;
            } else {
        ...
        if (ext->io_static) {
            ...
            npkt->req_complete++;
            if (ext->ion->
                tx_up(ext->reg_hdl, npkt, 0, 0, ext->cell, ext->cfg.lan,
                      0) == 0) {
                mpc_receive_complete(npkt, ext, ext);
            }
        } else {
            if (npkt =
                ext->ion->tx_up_start(ext->reg_hdl, npkt, 0, 0, ext->cell,
                                      ext->cfg.lan, 0, ext)) {
                mpc_receive_complete(npkt, ext, ext);
            }
        }
