...
#include <stdlib.h>
#include <sys/file.h>
#if !defined(__sgi) && !defined(__QNXNTO__)
#include <kvm.h>
#endif
#include <fcntl.h>
...
static const char rcsid[] = "@(#)$Id: kmem.c,v 1.5 2002/09/07 04:13:07 xtang Exp $";
#endif
...
char	*kern, *core;
{
	...
	if (ConnectServerInfo(0, fd, &sinfo) == -1) {
		perror("ConnectServerInfo");
		return -1;
	}
	
	if (ND_NODE_CMP(sinfo.nd, ND_LOCAL_NODE) == 0) {
		snprintf(pidname, _POSIX_PATH_MAX, "/proc/%d/as", sinfo.pid);
	} else {
		int len;
		
		if ((len = netmgr_ndtostr(ND2S_DIR_SHOW, sinfo.nd, pidname, _POSIX_NAME_MAX)) == -1)
		{
			perror("netmgr_ndtostr");
			return -1;
		}
		snprintf(&pidname[len], _POSIX_PATH_MAX - len, "proc/%d/as", sinfo.pid);
	}
	
	kvm_fd = open(pidname, O_RDONLY);
	if (kvm_fd == -1)
	{
...
register int	n;
{
	...
	if (kvm_fd == -1)
		if (openkmem(NULL, NULL) == -1)
			return -1;
	if (lseek(kvm_fd, pos, 0) == -1)
	{
		perror("kmemcpy:lseek");
		return -1;
	}
    while ((r = read(kvm_fd, buf, n)) < n)
#endif
		if (r <= 0)
		    {
