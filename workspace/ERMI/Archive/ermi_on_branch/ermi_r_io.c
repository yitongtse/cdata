    ...
    if (len <= 0) {
        ...
        if (count <= 0) {
            if (errno != EAGAIN) {
                ...
                // TBD
                // Need code here to handling queuing of 
                // messages to be sent later
            }
        } else if (count < len) { /* wrote partially */
            ERMI_R_DEBUG("ERMI-1 socket send completed partially\n");
            // TBD
            // Need code here to handling queue of partially
            // sent messages, to be sent later
        }
    }
...
int ermi_r_io_recv (ermi_r_neighbor_t *nbr, uchar *buf)
{
    ...
    if (count >= ERRP_HEADERBYTES) {
        ...
        // TBD
        // Need to add code here to handle partial reads
    } else {
