/* $Id: fsm.c,v 1.1 2010/10/01 17:41:01 ytse Exp ytse $
 * $Source: /Users/ytse/work/ermi/ios/RCS/fsm.c,v $
 *------------------------------------------------------------------
 * fsm.c -- General Purpose Finite State Machine Driver
 *
 * March 4, 1992 - Wayne Clark (Wayne's World. Wayne's World.. Party Out)
 *
 * Copyright (c) 1992-1996, 2004-2006 by cisco Systems, Inc.
 * All rights reserved.
 *------------------------------------------------------------------
 * $Log: fsm.c,v $
 * Revision 1.1  2010/10/01 17:41:01  ytse
 * Initial revision
 *
...
static void fsm_set_state_int (int *state, const int set_to)
{
    *state = set_to;
}

static int fsm_get_state_int (const int *state)
{
    return (*state);
}

static void fsm_set_state_unsigned_char (unsigned char *state,
					 const int set_to)
{
    *state = (unsigned char)set_to;
}

static int fsm_get_state_unsigned_char (const unsigned char *state)
{
    return (*state);
}

typedef void (*fsm_set_state_fn)(void *, int);
typedef int (*fsm_get_state_fn)(void *);

static short
fsm_execute_internal (
    char				*fsm_name,
    ...
    int					minor,
    boolean                    		debug,
    fsm_set_state_fn 			state_set,
    fsm_get_state_fn 			state_get)
{
    int   index;
    const int state = (*state_get)(state_in);
    ...
    */
    ...
    {
	(*state_set)(state_in, transition->next_state);
    }
...
fsm_execute (
    ...
    boolean                    		debug)
{
    return (fsm_execute_internal(fsm_name,
				 machine_table,
				 state,
				 max_state,
				 structure,
				 ...
				 minor,
				 debug,
				 (fsm_set_state_fn)fsm_set_state_int,
				 (fsm_get_state_fn)fsm_get_state_int));
}

/*
 * fsm_execute_lite
 *
 * As above, but with a shorter state field, to allow clients to avoid
 * wasting an int when maintaining state.
 */
short
fsm_execute_lite (
    char				*fsm_name,
    ...
    boolean                    		debug)
{
    return (fsm_execute_internal(
			    ...
			    minor,
			    debug,
			    (fsm_set_state_fn)fsm_set_state_unsigned_char,
			    (fsm_get_state_fn)fsm_get_state_unsigned_char));
}
