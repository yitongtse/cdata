...
#define MSG_TYPE_VIDEO_GET_PSI_TABLE	902
...
int main (int argc, char **argv)
{
    ...
    ipc_hdr->type = MSG_TYPE_VIDEO_GET_PSI_TABLE;;
    ipc_hdr->size = 0;
    ...
    n = read(msg_fd, msg_buf, 10000);
    close(msg_fd);

    // Generate web-page
    char filename[100];
    printf("Context-type:text/html\n\n");
    sprintf(filename, "%s/psi_table.html", TEMPLATE_DIR);
