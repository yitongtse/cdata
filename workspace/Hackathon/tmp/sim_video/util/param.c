...
static char* skip_to_param (param_t* par, const char* label)
{
    char* str;
    while (1) {
        /* get parameter */
...
void param_read (param_t* par, const char* filename)
{
    par->help_flag = par->inter_flag = par->echo_flag = 0;
...
void param_comment (param_t* par, const char* comment)
{
...
void param_get_int (param_t* par, const char* label, int* var, int default_val)
{
    if (par->help_flag) {
        printf("* %s (int) * %d\n", label, default_val);
        *var = default_val;
...
void param_get_int64 (param_t* par, const char* label, int64_t* var,
                      int64_t default_val)
{
    if (par->help_flag) {
        printf("* %s (int64) * %" PRIi64 "\n", label, default_val);
        *var = default_val;
        return;
    }
    ...
    // Get parameter
    if (par->inter_flag) {
        printf("%s (int %" PRIi64 "): ", label, default_val);
        fgets(par_line, MAX_LINE_WIDTH, stdin);
        if (strlen(par_line)) {
            sscanf(par_line, "%" PRIi64 "", var);
        } else {
            *var = default_val;
        }
    } else {
        sscanf(skip_to_param(par, label), "%" PRIi64, var);
    }

    if (par->echo_flag) {
        printf("* %s (int) * %" PRIi64 "n", label, *var);
    }
}


void param_get_float (param_t* par, const char* label, float* var, float default_val)
{
    if (par->help_flag) {
        printf("* %s (float) * %f\n", label, default_val);
        *var = default_val;
...
void param_get_string (param_t* par, const char* label, char* var, 
