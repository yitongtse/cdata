...
void dump_pmt_header (pmt_header_t *hdr)
{
    ...
    printf("Private           : %d\n", hdr->priv);
    printf("Sector length     : %d\n", hdr->sect_len);
    printf("Program number    : %d\n", (hdr->prog_num_1 << 8) | hdr->prog_num_2);
    printf("Version           : %d\n", hdr->ver);
    printf("Current/Next      : %d\n", hdr->cur_next);
    printf("Sector number     : %d\n", hdr->sect_num);
    printf("Last sector number: %d\n", hdr->last_sect_num);
...
{
    printf("PID number  : %d\n", (info->pid_hi << 8) | info->pid_lo);
    printf("ES type     : %d\n", info->es_type);
    printf("ES info size: %d\n\n", (info->es_info_len_hi << 8) | info->es_info_len_lo);
}


boolean parse_pat (uint8* buf, tp_header_t *tp_hdr, int* pmt_pid)
{
    int skip_sz;
    uint32 hdr[2];
    ...
    // dump_bytes((uint8*)psi_buf, left_bytes);
    ...
    while (left_bytes > 0) {
        ...
        if (pat_prog->prog_num != 0) {
            *pmt_pid = pat_prog->pmt_pid;   // first PMT pid found!
            return TRUE;
        }
    }
...
boolean parse_pmt (uint8* buf, tp_header_t *tp_hdr)
{
    int skip_sz;
    uint32 hdr[3];
    ...
    memcpy(psi_buf, tp_ptr, left_bytes);
    // dump_bytes(psi_buf2, left_bytes);

    hdr[0] = htonl(psi_buf[0]);
    hdr[1] = htonl(psi_buf[1]);
    hdr[2] = htonl(psi_buf[2]);
    pmt_header_t* pmt_hdr = (pmt_header_t*)hdr;

    printf("\n\nPMT:\n");
    dump_pmt_header(pmt_hdr);
    left_bytes = pmt_hdr->sect_len + 3 - 12 - 4;

    int psi_buf_offset = 12 + pmt_hdr->prog_info_len;
    left_bytes -= pmt_hdr->prog_info_len;

    while (left_bytes > 0) {
        memcpy(hdr, psi_buf2 + psi_buf_offset, 8);
        // dump_bytes(hdr, 8);
        hdr[0] = htonl(hdr[0]);
        hdr[1] = htonl(hdr[1]);
        pmt_es_info_t* es_info = (pmt_es_info_t*)hdr;
        dump_pmt_es_info(es_info);
        int es_info_len = (es_info->es_info_len_hi << 8) | es_info->es_info_len_lo;
        left_bytes -= 5 + es_info_len;
        psi_buf_offset += 5 + es_info_len;
    }
...
int main (int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if (tp_hdr->pid == 0) {
            boolean rc = parse_pat((uint8*)tp_buf, tp_hdr, &pmt_pid);
            printf("PAT parse result: %d\n", rc);
        }

        if (tp_hdr->pid == pmt_pid) {
            boolean rc = parse_pmt((uint8*)tp_buf, tp_hdr);
