...
typedef struct pat_header_ {
    ...
    uint32 resv1 : 2;
    uint32 priv : 1;
    uint32 sect_syntax : 1;
    uint32 table_id : 8;

    uint32 last_sect_num : 8;
    uint32 sect_num : 8;
    uint32 cur_next : 1;
    uint32 ver : 5;
    uint32 resv2 : 2;
...
// Set ES info length in PMT section
//
static inline
void es_info_set_info_len (pmt_es_info_t *es_info, uint16 len)
{
    es_info->es_info_len_hi = len >> 8;
    es_info->es_info_len_lo = len & 0xFF;
}
